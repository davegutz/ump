...
static int                      gb_grab,gb_sync,gb_bufcount;
...
#define BTTV_VERSION  	        _IOR('v' , BASE_VIDIOCPRIVATE+6, int)
	...
	if ((char*)-1 != map) {
	    ...
	    gb_bufcount = (MAX_BUFFERS < gb_buffers.frames) ?
		MAX_BUFFERS : gb_buffers.frames;
	} else {
	  if (debug) perror("grab_read_setparams");
...
grab_mm_capture(int single)
{
    ...
    if (!single) {
	/* queue up all available buffers */
...
/* Provide access to statics for initialization of ump program */
int
setStatics()
{
  if ( 0 < fd ) close(fd);
