...
read_config()
{
	...
	/*DAG	if (-1 != mixer_open(mixerdev, mixerctl))
	    have_mixer = 1;
	    else*/
	...
	mjpeg_quality = i;
    if (-1 != (i = cfg_get_int("global","toggle-mouse")))
	toggle_mouse = i;
    ...
    /* launch */
    /* Following line required for proper capture.  */
