...
* see http://math.nist.gov/. for latest updates.
*
*/
...
//  see ".req.h" for TNT requirements for underlying vector
...
#else
 ostream& operator<<(ostream &s, const Vector_Adaptor<BBVec> &A)
#endif // HAVE_STD
{
    Subscript M=A.size();
...
#else
 istream& operator>>(istream &s, Vector_Adaptor<BBVec> &A)
#endif // HAVE_STD
{
    Subscript N;
