...
{                    
  ...
  while (1)
	...
	case 'h':
	  ...
	  break;
	case 'V':
	  fprintf(stdout, "lptoutd: $Id: lptoutd.c,v 1.2 2010/01/10 11:37:14 davegutz Exp $ \n\
written by Dave Gutz Copyright (C) 2005");
  ...
  if (argc!=2)
    {
      fprintf(stderr, "ERROR(lptoutd): Wrong number of arguments. \n\
Needs one argument which is number between 0 and 255.\n");
  ...
  if (sscanf(argv[1],"%i",&value)!=1)
    {
      fprintf(stderr, "ERROR(lptoutd): Parameter is not a number.\n");
      exit(1);
    }
  if ((value<0) || (value>255))
    {
      fprintf(stderr,
	      "ERROR(lptoutd): Invalid value. \n\
Number must be between 0 and 255\n");
      exit(1);
    }
  int fdport = -1;
  if ( -1 == (fdport = open("/dev/port", O_RDWR)) )
    {
      fprintf(stderr, "ERROR(lptoutd): Couldn't get the port at /dev/port\n");
      exit(1);
    }
  if ( -1 == lseek(fdport, base, SEEK_SET))
    {
      perror("ERROR(lptoutd): lseek error");
      exit(1);
    }
  if ( -1 == write(fdport, &value, 1 ) )
    {
      perror("ERROR(lptoutd): write error");
      exit(1);
    }
  return(0);
}                                            
